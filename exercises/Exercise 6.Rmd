...
```{r}
Ed_table <- with(NHANESdata, table(Education))
prop.table(Ed_table)*100
Ed_barchart <- as.data.frame(prop.table(Ed_table)*100)
ggplot(Ed_barchart, aes(Education, Freq))+
  geom_col()
#Wasn't sure if you wanted a bar chart of raw counts or percentages so I included both
ggplot(NHANESdata, aes(x=Education)) + geom_bar(fill = "purple") + labs(title = "", x="", y="")
```
...
ggplot(NHANESdata,aes(x=BMIcat, fill = Gender) ) +
...
```{r}
ggplot(NHANESdata, aes(x=ArmCircumf))+
  geom_histogram(bins = 20, color="white",fill="steelblue")+
  labs(title = "colored histogram with 20 bins", x="", y="")
```
...
ggplot(NHANESdata, aes(x="", y=DR1TPROT))+
  geom_boxplot()+
  labs(y="Total protein consumed daily in grams", x="", title="Box Plot")
